...
export default function DashboardPage() {
  ...
  const filtered = useMemo(() => {
    if (!coupons) return [];
    return campaignFilter === 'all' ? coupons : coupons.filter(c => c.campaignId === campaignFilter);
  ...
  const dailyData = useMemo(() => {
    ...
    filtered.filter(c => c.status === 'claimed' && c.claimedAt).forEach(c => {
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <KpiCard title="Total Uploaded" value={metrics.totalUploaded} icon={<Upload className="h-5 w-5" />} />
